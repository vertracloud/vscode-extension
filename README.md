...
2. Connect your account:
   - **Connect account** opens your browser, you sign in and approve the access, and you are back in the editor.
   - **Use API key** is for machines without a browser. Create a key in the dashboard under **Settings → API keys** and paste it when asked. See [scopes](#api-key-scopes) for what to grant.
...
When you connect with an API key, grant only what you use. With **Connect account**, you choose the same scopes on the approval screen.
...
- The session token or API key is kept only in the editor's secret storage — never in settings, workspace files or logs.
- The extension sends requests only to `api.vertracloud.app`. Signing in opens your browser and receives the answer on a local port.
...
| Area | Commands |
|---|---|
| Account | Connect account · Use API key · Disconnect · Refresh · Show service status · Open dashboard · Open documentation · Copy ID |
